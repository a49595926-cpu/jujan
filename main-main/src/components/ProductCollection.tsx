  ...
  return (
    <section
      ...
      className={`relative overflow-hidden ${
        compact ? 'pt-6 pb-16 sm:pt-8 sm:pb-22' : 'py-16 sm:py-22'
      }`}
    >
      ...
      <div className="relative mx-auto max-w-8xl px-5 sm:px-8">
        ...
        <div className="mt-10 grid auto-rows-fr gap-4 sm:mt-11 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3">
...
export function ProductCard({ product, delay }: { product: Product; delay: number }) {
  ...
  const handleCardClick = () => {
    if (product.category === 'saffron') return;
  ...
  return (
    <article
      ...
      className={`reveal ${isVisible ? 'is-visible' : ''} group relative flex h-full flex-col overflow-hidden rounded-3xl border border-saffron-500/15 bg-espresso-800/50 transition-all duration-500 ${
        product.category !== 'saffron' ? 'cursor-pointer' : ''
      } hover:-translate-y-1.5 ${accentRing} hover:shadow-gold-lg`}
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        <h3 className="font-serif text-2xl leading-tight text-cream-50">
          {product.name}
        </h3>
        ...
        <p className="mt-2 flex-1 text-sm leading-relaxed text-cream-300">
          {product.description}
        </p>
        ...
        <ul className="mt-3 flex flex-wrap gap-x-4 gap-y-1.5">
          {product.features.map((f) => (
            <li key={f} className="flex items-center gap-1.5 text-xs text-cream-300">
              <Check
                className="h-3.5 w-3.5 text-saffron-400"
                strokeWidth={2.5}
              />
              {f}
            </li>
        ...
        <div className="mt-3.5 flex items-end justify-between border-t border-saffron-500/10 pt-3.5">
          <div>
            <p className="text-[10px] uppercase tracking-widest text-cream-400">
              From {product.origin}
            </p>
